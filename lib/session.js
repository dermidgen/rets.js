 ...
 */
 Session.prototype.login = function () {
    var $this = this;
    debug("Current Time              : %o", new Date());
    debug("Session Timeout Time Stamp: %o", new Date(this.timeoutTimeStamp*1000));
    var expired = this.expired();
    debug("Session Expired?: %o", expired);
    ...
    if (!expired) {
        ...
        var req = request(this, 'Login', {}, function(err){
            $this.emit('error',err);
        }).pipe(response()).on('capability',function(key, value){
            ...
            if (key === 'TimeoutSeconds') {
                debug("$this.timeoutTimeStamp: %o", $this.timeoutTimeStamp);
                $this.timeoutTimeStamp = new Date().getTime() + parseInt(value);
                debug("$this.timeoutTimeStamp: %o", $this.timeoutTimeStamp);
            }
        }).on('finish',function(){
            debug(".login()::finish");
...
/**
 * Calculate Digest hash for RETS-UA-Authorization headers
 * @return {boolean} Has the session expired? true or false
 */
Session.prototype.expired = function () {
    var expired = new Date().getTime() < this.timeoutTimeStamp;
    return !expired;
};
