 ...
 */
 Session.prototype.login = function () {
    var $this = this;
    debug("Current Time: %o", new Date().getTime());
    debug("Session Timeout Time Stamp: %o", $this.timeoutTimeStamp);
    debug("Session Expired?: %o", this.expired());

    if (this.expired()) {
        return request(this, 'Login', {}, function(err){
            $this.emit('login',err);
        })
        .pipe(response())
        ...
        .on('finish',function(){
            debug("Session.login() finished.");
            // debug("Session: %s", util.inspect($this, {colors:true, depth:1}));
...
Session.prototype.expired = function () {
    return new Date().getTime() < this.timeoutTimeStamp;
};
