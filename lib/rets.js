...
RETS.prototype.getMetadata = function (options) {
    ...
    debug("Metadata Time Stamp: %s", MetadataTimeStamp);
    debug("Metadata Cache File: %s", file);
    var exists = fs.existsSync(file);
    debug("Metadata Cache File already exists: %s", exists);
    if ( exists ) {
        var CachedMetadataTimeStamp = null;
        cache = JSON.parse(fs.readFileSync(file));
        CachedMetadataTimeStamp = new Date(cache.timestamp);
        debug("Cached Metadata Time Stamp: %s", CachedMetadataTimeStamp);
        var stale = (MetadataTimeStamp.getTime() > CachedMetadataTimeStamp.getTime());
            ...
            parser.addListener('end', function(result) {
                ...
                fs.writeFile(file, JSON.stringify(cache, null, 4), function(err) {
                    if(err) {debug(err);} else {
                        debug("Metadata Cache successfully written to: %s", file);
                    }
                });
